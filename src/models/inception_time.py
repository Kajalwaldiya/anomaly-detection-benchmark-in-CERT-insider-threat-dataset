...
learn = ts_learner(dls, arch='InceptionTime', metrics=[accuracy, F1Score(average='macro')])
...
fig_cm.savefig(os.path.join(RESULTS_PATH, "confusion_matrix_inception_time.png"), bbox_inches="tight")
...
ax_roc.legend()
ax_roc.grid(True)
fig_roc.savefig(os.path.join(RESULTS_PATH, "roc_curve_inception_time.png"), bbox_inches="tight")

# 4. Save Loss Curve
fig_loss, ax_loss = plt.subplots()
learn.recorder.plot_loss(ax=ax_loss)
fig_loss.savefig(os.path.join(RESULTS_PATH, "loss_curve_inception_time.png"), bbox_inches="tight")
...
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(RESULTS_PATH, "pr_curve_inception_time.png"), bbox_inches="tight")